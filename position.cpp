...
Position::Position(const Position &s)
    : us(s.us), them(s.them), fiftyMoveRule(s.fiftyMoveRule),
      castleRights(s.castleRights), phase(s.phase), key(s.key),
      pawnKey(s.pawnKey), checkers(s.checkers), enPassant(s.enPassant),
      previousMove(s.previousMove), checkSquares(s.checkSquares),
      pinned(s.pinned), occupancy(s.occupancy), pieceIndex(s.pieceIndex),
      board(s.board), pieces(s.pieces), pieceCounts(s.pieceCounts),
...
void Position::setPins(Color c) {
    ...
    pinners = bishopMoves[kingSq] & (getPieceBB<PIECETYPE_BISHOP>(!c) |
...
bool Position::isLegal(Move move) const {
    ...
    if (square_bb[src] & pinned[us] &&
        !(coplanar[src][dst] & getPieceBB<PIECETYPE_KING>(us))) {
        return false;
    }

    if (onSquare(src) == PIECETYPE_PAWN && square_bb[dst] & enPassant) {
        U64 change = us == WHITE ? square_bb[src] | square_bb[dst - 8]
...
bool Position::isValid(Move move, U64 validMoves) const {
    ...
    if (!(square_bb[src] & getOccupancyBB(us)) ||
        (square_bb[dst] & getOccupancyBB(us)) || dst == getKingSquare(them)) {
        return false;
    }
...
bool Position::givesCheck(Move move) const {
    ...
    // Discovered check
    if ((getDiscoveredChecks(us) & square_bb[src]) &&
        !(coplanar[src][dst] & getPieceBB<PIECETYPE_KING>(them))) {
        return true;
    }
    ...
    if (isEnPassant(move)) {
        ...
        return getAttackBB(PIECETYPE_ROOK, rookSquare,
                           getOccupancyBB() ^ square_bb[src]) &
               getPieceBB<PIECETYPE_KING>(them);
    }
...
int Position::see(Move move) const {
    ...
    if (onSquare(src) == PIECETYPE_PAWN && square_bb[dst] & enPassant) {
        ...
        gain[d] = PIECE_VALUE[PIECETYPE_QUEEN].score() -
                  PIECE_VALUE[PIECETYPE_PAWN].score();
    }
    else {
...
bool Position::insufficientMaterial() const {
    ...
    if (getPieceCount<PIECETYPE_PAWN>() + getPieceCount<PIECETYPE_ROOK>() +
            getPieceCount<PIECETYPE_QUEEN>() ==
        0) {
        switch (getPieceCount<PIECETYPE_KNIGHT>()) {
            case 0:
                if ((getPieceBB<PIECETYPE_BISHOP>() & DARK_SQUARES) ==
                        getPieceBB<PIECETYPE_BISHOP>() ||
                    (getPieceBB<PIECETYPE_BISHOP>() & LIGHT_SQUARES) ==
                        getPieceBB<PIECETYPE_BISHOP>()) {
                    res = true;
                }
                break;
            ...
            case 2:
                if (getPieceCount<PIECETYPE_KNIGHT>(WHITE) &&
                    getPieceCount<PIECETYPE_KNIGHT>(BLACK) &&
                    !getPieceCount<PIECETYPE_BISHOP>())
                    res = true;
                break;
...
std::string Position::getFen() { // Current FEN
    ...
    bool enpass = false;
    if (moved == PIECETYPE_PAWN &&
        int(std::max(src, dst)) - int(std::min(src, dst)) == 16) {
        for (Square p : getPieceList<PIECETYPE_PAWN>(us)) {
            if (rank(p) == rank(dst) &&
                std::max(file(p), file(dst)) - std::min(file(p), file(dst)) == 1) {
