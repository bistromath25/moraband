...
public:
    ...
    std::size_t size() { return game_history.size(); }
    void init() {
        killers = {};
        history = {};
        game_history.clear();
    ...
    void push(const std::pair<Move, U64> &p) { game_history.push_back(p); }
    void pop() {
    ...
    const std::pair<Move, Move> &getKiller(int ply) const { return killers[ply]; }
    int getHistoryScore(Move move) const {
        assert(butterfly[getSrc(move)][getDst(move)] > 0);
        return history[getSrc(move)][getDst(move)] /
               butterfly[getSrc(move)][getDst(move)];
    }
