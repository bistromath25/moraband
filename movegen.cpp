...
void mg_init() { initmagicmoves(); }

std::size_t MoveList::size() const { return sz; }

void MoveList::push(Move move) { moveList[sz++].move = move; }

Move MoveList::pop() { return moveList[--sz].move; }

bool MoveList::contains(Move move) const {
    return std::find(moveList.begin(), moveList.begin() + sz, move) !=
           moveList.begin() + sz;
}
...
void MoveList::pushPromotion(Square src, Square dst) {
    ...
    if (square_bb[dst] & NOT_A_FILE &&
        square_bb[dst + 1] & position.getOccupancyBB(!c)) {
...
void MoveList::pushMoves() {
    ...
    for (Square src : position.getPieceList<P>(c)) {
        ...
        if (P == PIECETYPE_KING && T != MoveType::Attacks &&
            T != MoveType::Evasions) {
            pushCastle<T>();
        }
    }
...
void MoveList::pushPawnMoves() {
    ...
    if (T != MoveType::Attacks) {
        ...
        U64 dbl = (C == WHITE ? (up & RANK_3) << 8 : (up & RANK_6) >> 8) & empty;
        ...
        if (T == MoveType::QuietChecks) {
            ...
            up |= dis;
            dbl |= (C == WHITE ? (dis & RANK_3) << 8 : (dis & RANK_6) >> 8) & empty;
        }
...
void MoveList::pushCastle() {
    ...
    if (position.canCastleKingside() &&
        !(between_hor[k][k - 3] & position.getOccupancyBB()) &&
        !position.attacked(k - 1) && !position.attacked(k - 2)) {
        if (T == MoveType::QuietChecks) {
    ...
    if (position.canCastleQueenside() &&
        !(between_hor[k][k + 4] & position.getOccupancyBB()) &&
        !position.attacked(k + 1) && !position.attacked(k + 2)) {
        if (T == MoveType::QuietChecks) {
            if (square_bb[k + 1] & position.getCheckSquaresBB(PIECETYPE_ROOK)) {
                push(makeCastle(k, k + 2));
...
void MoveList::generateMoves() {
    ...
    valid = T == MoveType::Attacks
                ? position.getOccupancyBB(position.getTheirColor())
                : position.getEmptyBB();

    position.getOurColor() == WHITE ? pushPawnMoves<T, WHITE>()
...
void MoveList::generateMoves<MoveType::Evasions>() {
    ...
    valid =
        position.getOccupancyBB(position.getTheirColor()) | position.getEmptyBB();
    ...
    position.getOurColor() == WHITE ? pushPawnMoves<MoveType::Evasions, WHITE>()
...
void MoveList::generateMoves<MoveType::All>() {
    ...
    position.getOurColor() == WHITE ? pushPawnMoves<MoveType::All, WHITE>()
    ...
    switch (stage) {
        ...
        case AttacksGen:
            ...
            for (int i = 0; i < int(sz); ++i) {
                ...
                if (see >= 0) {
                    moveList[i].score = position.onSquare(getDst(moveList[i].move)) -
                                        position.onSquare(getSrc(moveList[i].move));
                }
                else {
        ...
        case Attacks:
            ...
            // seen before!
            while (sz) {
                std::iter_swap(std::max_element(moveList.begin(), moveList.begin() + sz),
                               moveList.begin() + sz - 1);
                move = pop();
        ...
        case Killer1:
            ...
            stage++;
            if ((position.isValid(killer1, FULL) && position.isLegal(killer1) &&
                 position.isQuiet(killer1)) &&
                killer1 != best) {
                return killer1;
            }
        ...
        case Killer2:
            ...
            stage++;
            if ((position.isValid(killer2, FULL)) && position.isLegal(killer2) &&
                position.isQuiet(killer2) && killer2 != best && killer2 != killer1) {
                return killer2;
            }
        ...
        case QuietsGen:
            ...
            // based on their PST score.
            {
                ...
                auto it2 =
                    std::partition(moveList.begin(), moveList.begin() + sz, noScore);
                ...
                for (auto it1 = moveList.begin(); it1 != it2; ++it1) {
                    ...
                    it1->score =
                        PieceSquareTable::getTaperedScore(toMove, position.getOurColor(),
                                                          dst, position.getGamePhase()) -
                        PieceSquareTable::getTaperedScore(toMove, position.getOurColor(),
                                                          src, position.getGamePhase());
                }
        ...
        case Quiets:
            ...
            while (sz) {
                move = pop();
                if (move != best && move != killer1 && move != killer2 &&
        ...
        case EvadeBestMove:
            ...
            // If the move given by the pv or tt is valid, return it.
            stage++;
            if (position.isValid(best, valid) && position.isLegal(best) &&
                !position.inDoubleCheck()) {
                return best;
            }
                ...
                for (int i = 0; i < int(sz); ++i) {
                    if (position.isCapture(moveList[i].move)) {
                        ...
                        if (hist > 0) {
                            ...
                            moveList[i].score =
                                PieceSquareTable::getTaperedScore(piece, position.getOurColor(),
                                                                  dst,
                                                                  position.getGamePhase()) -
                                PieceSquareTable::getTaperedScore(piece, position.getOurColor(),
                                                                  src, position.getGamePhase());
                        }
                    }
...
MoveList::MoveList(const Position &s, Move bestMove, History *history, int ply,
                   bool qsearch)
    : isQSearch(qsearch), valid(FULL), position(s), history(history), ply(ply),
      sz(0), best(bestMove), killer1(NULL_MOVE), killer2(NULL_MOVE) {
    if (history) {
...
/* List of moves and related functions */
MoveList::MoveList(const Position &s)
    : isQSearch(false), valid(FULL), position(s), history(nullptr), ply(0),
      sz(0), best(NULL_MOVE) {
    generateMoves<MoveType::All>();
    checkLegal();
    stage = AllLegal;
