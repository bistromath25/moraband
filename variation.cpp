...
int Variation::size() const { return sz; }
...
void Variation::pushToPv(Move move, U64 key, int ply, int score) {
    ...
    std::copy(std::make_move_iterator(pv.begin() + copyFromStart),
              std::make_move_iterator(pv.begin() + copyFromEnd),
              pv.begin() + copyTo);
}

U64 Variation::getPvKey(int ply) const { return pv[ply].second; }

Move Variation::getPvMove(int ply) const { return pv[ply].first; }

bool Variation::isMate() const { return isMatingLine; }

int Variation::getMateInN() const { return sz / 2.0 + 0.5; }
...
int Variation::getIndex(int ply) { return ply * (2 * MAX_PLY + 1 - ply) / 2; }