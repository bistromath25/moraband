...
private:
    ...
    std::array<std::array<std::array<Square, PIECE_MAX>, PIECE_TYPES_SIZE>,
               PLAYER_SIZE>
        pieceList;
};

inline Color Position::getOurColor() const { return us; }

inline Color Position::getTheirColor() const { return them; }

inline int Position::getFiftyMoveRule() const { return fiftyMoveRule; }

inline U64 Position::getKey() const { return key; }

inline U64 Position::getPawnKey() const { return pawnKey; }

inline U64 Position::getEnPassantBB() const { return enPassant; }

inline int Position::getCastleRights() const { return castleRights; }

inline U64 Position::getPinsBB(Color c) const { return pinned[c]; }

inline int Position::getGamePhase() const { return phase; }

inline void Position::setGamePhase() {
    int p = totalPhase - getPieceCount<PIECETYPE_PAWN>() * pawnPhase -
...
inline bool Position::isEnPassant(Move move) const {
    return onSquare(getSrc(move)) == PIECETYPE_PAWN &&
           (square_bb[getDst(move)] & enPassant);
}

template<PieceType P>
inline const std::array<Square, PIECE_MAX> &
Position::getPieceList(Color c) const {
...
inline int Position::getNonPawnPieceCount(Color c) const {
    return pieceCounts[c][PIECETYPE_KNIGHT] + pieceCounts[c][PIECETYPE_BISHOP] +
           pieceCounts[c][PIECETYPE_ROOK] + pieceCounts[c][PIECETYPE_QUEEN];
}
...
inline void Position::setCheckers() {
    ...
    checkSquares[PIECETYPE_PAWN] =
        getAttackBB<PIECETYPE_PAWN>(getKingSquare(them), them);
    checkSquares[PIECETYPE_KNIGHT] =
        getAttackBB<PIECETYPE_KNIGHT>(getKingSquare(them));
    checkSquares[PIECETYPE_BISHOP] =
        getAttackBB<PIECETYPE_BISHOP>(getKingSquare(them));
    checkSquares[PIECETYPE_ROOK] =
        getAttackBB<PIECETYPE_ROOK>(getKingSquare(them));
    checkSquares[PIECETYPE_QUEEN] =
        checkSquares[PIECETYPE_BISHOP] | checkSquares[PIECETYPE_ROOK];
}
...
inline bool Position::defended(Square s, Color c) const {
    return getAttackBB<PIECETYPE_PAWN>(s, !c) & getPieceBB<PIECETYPE_PAWN>(c) ||
           ...
           getAttackBB<PIECETYPE_BISHOP>(s) & (getPieceBB<PIECETYPE_BISHOP>(c) |
                                               getPieceBB<PIECETYPE_QUEEN>(c)) ||
           getAttackBB<PIECETYPE_ROOK>(s) &
               (getPieceBB<PIECETYPE_ROOK>(c) | getPieceBB<PIECETYPE_QUEEN>(c));
}
...
inline bool Position::isAttacked(Square sq, Color c, U64 change) const {
    ...
    return getAttackBB<PIECETYPE_PAWN>(sq, c) & getPieceBB<PIECETYPE_PAWN>(!c) ||
           ...
           Bmagic(sq, occupancy) & (getPieceBB<PIECETYPE_BISHOP>(!c) |
                                    getPieceBB<PIECETYPE_QUEEN>(!c)) ||
           Rmagic(sq, occupancy) & (getPieceBB<PIECETYPE_ROOK>(!c) |
                                    getPieceBB<PIECETYPE_QUEEN>(!c)) ||
           getAttackBB<PIECETYPE_KING>(sq) & getPieceBB<PIECETYPE_KING>(!c);
}

inline bool Position::attacked(Square sq) const {
    return getAttackBB<PIECETYPE_PAWN>(sq, us) &
               getPieceBB<PIECETYPE_PAWN>(them) ||
           getAttackBB<PIECETYPE_KNIGHT>(sq) &
               getPieceBB<PIECETYPE_KNIGHT>(them) ||
           getAttackBB<PIECETYPE_BISHOP>(sq) &
               (getPieceBB<PIECETYPE_BISHOP>(them) |
                getPieceBB<PIECETYPE_QUEEN>(them)) ||
           getAttackBB<PIECETYPE_ROOK>(sq) & (getPieceBB<PIECETYPE_ROOK>(them) |
                                              getPieceBB<PIECETYPE_QUEEN>(them));
}

inline bool Position::inCheck() const { return getCheckersBB(); }

inline bool Position::inDoubleCheck() const {
    return pop_count(getCheckersBB()) == 2;
}

inline bool Position::check() const { return attacked(getKingSquare(us)); }

inline U64 Position::getAttackersBB(Square sq, Color c) const {
    return (getAttackBB<PIECETYPE_PAWN>(sq, !c) & getPieceBB<PIECETYPE_PAWN>(c)) |
           (getAttackBB<PIECETYPE_KNIGHT>(sq) & getPieceBB<PIECETYPE_KNIGHT>(c)) |
           (getAttackBB<PIECETYPE_BISHOP>(sq) &
            (getPieceBB<PIECETYPE_BISHOP>(c) | getPieceBB<PIECETYPE_QUEEN>(c))) |
           (getAttackBB<PIECETYPE_ROOK>(sq) &
            (getPieceBB<PIECETYPE_ROOK>(c) | getPieceBB<PIECETYPE_QUEEN>(c)));
}
...
inline U64 Position::allAttackers(Square sq) const {
    return (getAttackBB<PIECETYPE_PAWN>(sq, us) &
            getPieceBB<PIECETYPE_PAWN>(them)) |
           (getAttackBB<PIECETYPE_PAWN>(sq, them) &
            getPieceBB<PIECETYPE_PAWN>(us)) |
           (getAttackBB<PIECETYPE_KNIGHT>(sq) & getPieceBB<PIECETYPE_KNIGHT>()) |
           (getAttackBB<PIECETYPE_BISHOP>(sq) &
            (getPieceBB<PIECETYPE_BISHOP>() | getPieceBB<PIECETYPE_QUEEN>())) |
           (getAttackBB<PIECETYPE_ROOK>(sq) &
            (getPieceBB<PIECETYPE_ROOK>() | getPieceBB<PIECETYPE_QUEEN>())) |
           (getAttackBB<PIECETYPE_KING>(sq) & getPieceBB<PIECETYPE_KING>());
}

inline bool Position::check(U64 change) const {
    return (Bmagic(getKingSquare(us), getOccupancyBB() ^ change) &
            (getPieceBB<PIECETYPE_BISHOP>(them) |
             getPieceBB<PIECETYPE_QUEEN>(them))) ||
           (Rmagic(getKingSquare(us), getOccupancyBB() ^ change) &
            (getPieceBB<PIECETYPE_ROOK>(them) |
             getPieceBB<PIECETYPE_QUEEN>(them)));
}

inline bool Position::check(U64 change, Color c) const {
    return (Bmagic(getKingSquare(c), getOccupancyBB() ^ change) &
            (getPieceBB<PIECETYPE_BISHOP>(!c) |
             getPieceBB<PIECETYPE_QUEEN>(!c))) ||
           (Rmagic(getKingSquare(c), getOccupancyBB() ^ change) &
            (getPieceBB<PIECETYPE_ROOK>(!c) | getPieceBB<PIECETYPE_QUEEN>(!c)));
}
...
inline Move Position::getPreviousMove() const { return previousMove; }
