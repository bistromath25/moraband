...
const Score PIECE_VALUE[7] = {
    PAWN_WEIGHT, KNIGHT_WEIGHT, BISHOP_WEIGHT, ROOK_WEIGHT,
    QUEEN_WEIGHT, KING_WEIGHT, S(0, 0) // none
};

Score KNIGHT_MOBILITY[9] = {S(-50, -60), S(-30, -30), S(-10, -10),
                            S(0, 0), S(10, 10), S(20, 20),
                            S(25, 25), S(30, 30), S(35, 35)};

Score BISHOP_MOBILITY[14] = {S(-30, -40), S(-15, -15), S(-5, -5), S(0, 0),
                             S(5, 5), S(10, 10), S(15, 15), S(20, 20),
                             S(25, 25), S(30, 30), S(35, 35), S(40, 40),
                             S(45, 45), S(50, 50)};

Score ROOK_MOBILITY[15] = {S(-30, -40), S(-15, -15), S(-5, -5), S(0, 0),
                           S(5, 10), S(10, 20), S(15, 30), S(20, 40),
                           S(25, 50), S(30, 55), S(35, 60), S(40, 65),
                           S(45, 70), S(50, 75), S(50, 80)};

Score QUEEN_MOBILITY[28] = {
    S(-30, -40), S(-20, -20), S(-10, -10), S(0, 0), S(5, 5), S(10, 10),
    S(15, 15), S(20, 20), S(25, 25), S(30, 30), S(35, 35), S(40, 40),
    S(45, 45), S(50, 50), S(50, 55), S(50, 60), S(50, 65), S(50, 70),
    S(50, 75), S(50, 80), S(50, 85), S(50, 90), S(50, 90), S(50, 90),
    S(50, 90), S(50, 90), S(50, 90), S(50, 90)};
...
Score PAWN_PASSED[4][7] = {
    ...
    {// Protected advance
     S(0, 0), S(7, 7), S(12, 12), S(17, 22), S(30, 45), S(80, 100),
     S(115, 200)},
    {// Safe advance
     S(0, 0), S(8, 8), S(15, 15), S(20, 24), S(40, 60), S(100, 190),
     S(150, 290)}};
...
Evaluate::Evaluate(const Position &s)
    : position(s), mobility{}, king_safety{}, pawn_structure{}, material{},
      attacks{}, piece_attacks_bb{}, all_attacks_bb{} {
    // Tapered-evaluation with 256 phases, 0 (Opening/Middlegame) and 255
    // (Endgame)
    ...
    // Pawn attacks
    piece_attacks_bb[WHITE][PIECETYPE_PAWN] |=
        (position.getPieceBB<PIECETYPE_PAWN>(WHITE) & NOT_A_FILE) << 9 &
        position.getOccupancyBB();
    piece_attacks_bb[WHITE][PIECETYPE_PAWN] |=
        (position.getPieceBB<PIECETYPE_PAWN>(WHITE) & NOT_H_FILE) << 7 &
        position.getOccupancyBB();
    piece_attacks_bb[BLACK][PIECETYPE_PAWN] |=
        (position.getPieceBB<PIECETYPE_PAWN>(BLACK) & NOT_A_FILE) >> 7 &
        position.getOccupancyBB();
    piece_attacks_bb[BLACK][PIECETYPE_PAWN] |=
        (position.getPieceBB<PIECETYPE_PAWN>(BLACK) & NOT_H_FILE) >> 9 &
        position.getOccupancyBB();
    ...
    evalAttacks(WHITE);
    evalAttacks(BLACK);
    score = mobility[c] - mobility[!c] + king_safety[c] - king_safety[!c] +
            pawn_structure[c] - pawn_structure[!c] + material[c] - material[!c] +
            attacks[c] - attacks[!c];
    score += S(position.getPstScore(MIDDLEGAME), position.getPstScore(ENDGAME));
    score += TEMPO_BONUS;
}

int Evaluate::getScore() const { return score.score(gamePhase) + CONTEMPT; }
...
void Evaluate::evalOutposts(const Color c) {
    for (Square p : position.getPieceList<PIECETYPE_KNIGHT>(c)) {
        ...
        if (!(p & outpost_area[c]) ||
            !(pawn_attacks[!c][p] & position.getPieceBB<PIECETYPE_PAWN>(c)) ||
            in_front[c][p] & adj_files[p] &
                position.getPieceBB<PIECETYPE_PAWN>(!c)) {
            continue;
        }
        material[c] += KNIGHT_OUTPOST;
        if (!position.getPieceBB<PIECETYPE_KNIGHT>(!c) &&
            ((!position.getPieceBB<PIECETYPE_BISHOP>(!c)) & squares_of_color(p))) {
            material[c] += KNIGHT_OUTPOST;
        }
    }
...
void Evaluate::evalPawns(const Color c) {
    ...
    for (Square p : position.getPieceList<PIECETYPE_PAWN>(c)) {
        ...
        if (!((file_bb[p] | adj_files[p]) & in_front[c][p] &
              position.getPieceBB<PIECETYPE_PAWN>(!c))) {
        ...
        else if (!(file_bb[p] & in_front[c][p] &
                   position.getPieceBB<PIECETYPE_PAWN>(!c))) {
            int sentries, helpers;
            assert(p < 56 && p > 7);

            sentries = pop_count(pawn_attacks[c][p + dir] &
                                 position.getPieceBB<PIECETYPE_PAWN>(!c));
            if (sentries > 0) {
                helpers = pop_count(pawn_attacks[!c][p + dir] &
                                    position.getPieceBB<PIECETYPE_PAWN>(c));
                if (helpers >= sentries) {
                    pawn_structure[c] += PAWN_PASSED_CANDIDATE;
                }
                else if (!(~in_front[c][p] & adj_files[p] &
                           position.getPieceBB<PIECETYPE_PAWN>(c))) {
        ...
        else if (rank_bb[p - dir] & position.getPieceBB<PIECETYPE_PAWN>(c) &
                 adj_files[p]) {
...
void Evaluate::evalPieces(const Color c) {
    ...
    for (Square p : position.getPieceList<PIECETYPE_BISHOP>(c)) {
        ...
        material[c] +=
            BAD_BISHOP *
            pop_count(squares_of_color(p) & position.getPieceBB<PIECETYPE_PAWN>(c));
        if (moves & king_net_bb[!c][position.getKingSquare(!c)] & mobilityNet) {
    ...
    for (int pt = PIECETYPE_PAWN; pt <= PIECETYPE_QUEEN; ++pt) {
        king_safety[!c] -= KING_RING_ATTACK[0][pt] *
                           pop_count(kingRing1 & piece_attacks_bb[c][pt]);
        king_safety[!c] -= KING_RING_ATTACK[1][pt] *
                           pop_count(kingRing2 & piece_attacks_bb[c][pt]);
    }
}
...
void Evaluate::evalPawnShield(const Color c, U64 pawnShieldCloseMask) {
    ...
    U64 pawnShieldMissing =
        ~(pawnShieldClose | shift(pawnShieldFar, c == WHITE ? S : N)) &
        pawnShieldCloseMask;
...
void Evaluate::evalAttacks(const Color c) {
    ...
    attackedByPawn =
        piece_attacks_bb[!c][PIECETYPE_PAWN] &
        (position.getOccupancyBB(c) ^ position.getPieceBB<PIECETYPE_PAWN>(c));

    while (attackedByPawn) {
        Square to = pop_lsb(attackedByPawn);
        if (position.getAttackBB<PIECETYPE_PAWN>(to, c) &
            position.getPieceBB<PIECETYPE_PAWN>(!c) & all_attacks_bb[!c]) {
    ...
    hanging =
        all_attacks_bb[!c] & position.getOccupancyBB(c) & ~all_attacks_bb[c];
...
std::ostream &operator<<(std::ostream &os, const Evaluate &e) {
    ...
    os << "-------------------------------------------------------------\n"
       ...
       << "    |    Total    |\n"
       << "-------------------------------------------------------------\n"
       << "| Material        |" << std::setw(12) << e.material[c].score() << " |"
       << std::setw(12) << e.material[!c].score() << " |" << std::setw(12)
       << e.material[c].score() - e.material[!c].score() << " |\n"
       << "-------------------------------------------------------------\n"
       << "| Mobility        |" << std::setw(12) << e.mobility[c].score() << " |"
       << std::setw(12) << e.mobility[!c].score() << " |" << std::setw(12)
       << e.mobility[c].score() - e.mobility[!c].score() << " |\n"
       << "-------------------------------------------------------------\n"
       << "| Pawn Structure  |" << std::setw(12) << e.pawn_structure[c].score()
       << " |" << std::setw(12) << e.pawn_structure[!c].score() << " |"
       << std::setw(12)
       << e.pawn_structure[c].score() - e.pawn_structure[!c].score() << " |\n"
       << "-------------------------------------------------------------\n"
       << "| King Safety     |" << std::setw(12) << e.king_safety[c].score()
       << " |" << std::setw(12) << e.king_safety[!c].score() << " |"
       << std::setw(12) << e.king_safety[c].score() - e.king_safety[!c].score()
       << " |\n"
       << "-------------------------------------------------------------\n"
       << "| Attacks         |" << std::setw(12) << e.attacks[c].score() << " |"
       << std::setw(12) << e.attacks[!c].score() << " |" << std::setw(12)
       << e.attacks[c].score() - e.attacks[!c].score() << " |\n"
       << "-------------------------------------------------------------\n"
       << "| PST             |" << std::setw(12) << pstMid << " |"
       << std::setw(12) << pstLate << " |" << std::setw(12) << pstMid + pstLate
       << " |\n"
       << "-------------------------------------------------------------\n"
       << "| Total           |             |             |" << std::setw(12)
       << e.getScore() << " |\n"
            ...
            for (int j = 0; j < 8; ++j) {
                ...
                for (int y = i - ring; y <= i + ring; ++y) {
                    for (int x = j - ring; x <= j + ring; ++x) {
                        ...
                        else if (y == i - ring || y == i + ring || x == j - ring ||
                                 x == j + ring) {
