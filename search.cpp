...
inline int reverse_futility_pruning_margin(int depth, int eval,
                                           bool improving) {
...
int qsearch(Position &s, SearchInfo &si, GlobalInfo &gi, int ply, int alpha,
    ...
    if (gi.history.isThreefoldRepetition(s) || s.insufficientMaterial() ||
    ...
    if (tt_entry.getKey() == s.getKey()) {
        ...
        if (tt_flag == FLAG_EXACT || (tt_flag == FLAG_LOWER && tt_score >= beta) ||
    ...
    while ((m = moveList.getBestMove())) {
        ...
        if (!s.inCheck() && !s.givesCheck(m) && !s.isEnPassant(m) &&
            !isPromotion(m)) {
            int fScore =
                staticEval + 100 + PIECE_VALUE[s.onSquare(getDst(m))].score();
            if (fScore <= alpha) {
...
int search(Position &s, SearchInfo &si, GlobalInfo &gi, int depth, int ply,
    ...
    if (depth == 0 ||
        ply > MAX_PLY) { // Perform qsearch when regular search is completed
    ...
    if (!isPv) {                     // Evaluate current position statically if current node is NOT a
                                     // PV node
        if (tt_flag == FLAG_EXACT) { // Make use of previous TT score
            ...
            if (tt_hit) {
                if ((staticEval < tt_score && tt_flag == FLAG_LOWER) ||
                    (staticEval > tt_score && tt_flag == FLAG_UPPER)) {
                    staticEval = tt_score;
                }
            }
        }
    ...
    // bool improving = ply >= 2 && !s.inCheck() && staticEval >
    // gi.evalHistory[ply - 2];
    bool improving = ply >= 2 && gi.evalHistory[ply - 2] != NEG_INF
                         ? gi.evalHistory[ply] > gi.evalHistory[ply - 2]
                     : ply >= 4 && gi.evalHistory[ply - 4] != NEG_INF
                         ? gi.evalHistory[ply] > gi.evalHistory[ply - 4]
                         : true;
    ...
    // Pruning
    if (!isPv && !s.inCheck() && s.getNonPawnPieceCount() &&
        beta > -CHECKMATE_BOUND) {
        // Reverse futility pruning
        if (depth <= REVERSE_FUTILITY_DEPTH &&
            staticEval -
                    reverse_futility_pruning_margin(depth, staticEval, improving) >=
                beta) {
            return beta;
        }
        // Null move pruning
        // Make a null move and search to a reduced depth
        if (!isNull && depth >= NULL_MOVE_DEPTH &&
            staticEval + NULL_MOVE_MARGIN >= beta) {
            Position n;
            std::memmove(&n, &s, sizeof(s));
            n.makeNull();
            gi.history.push(std::make_pair(NULL_MOVE, n.getKey()));
            int nullScore = -search(n, si, gi,
                                    std::max(1, depth - null_move_pruning_reduction(
                                                            depth, staticEval, beta)),
                                    ply + 1, -beta, -beta + 1, false, true);
            gi.history.pop();
            if (nullScore >= beta) {
    ...
    // Probcut
    if (depth >= PROBCUT_DEPTH && beta > -CHECKMATE_BOUND &&
        staticEval >= beta - PROBCUT_MARGIN(depth)) {
        int p = beta + 300;
        int probcutScore =
            search(s, si, gi, depth - 3, ply + 1, p - 1, p, isPv, isNull);
        if (probcutScore >= p) {
            return probcutScore;
        }
    }
    ...
    // In case no best move was found, perform a shallower search to determine
    // which move to properly seach first
    if (!tt_move && !isNull && !s.inCheck() &&
        (isPv || staticEval + 100 >= beta) && depth >= 5) {
    ...
    while ((m = moveList.getBestMove())) {
        ...
        if (bestScore > -CHECKMATE_BOUND && legalMoves > 1 && !s.inCheck() &&
            !s.givesCheck(m) && !isPromotion(m) && s.getNonPawnPieceCount()) {
            // Futility pruning
            ...
            if (depth <= 3 &&
                s.see(m) <
                    -PIECE_VALUE[PIECETYPE_PAWN].score() * (1 << (depth - 1)) &&
                m != tt_move) {
                continue;
            }
            // Late move reduction
            if (depth >= LATE_MOVE_REDUCTION_DEPTH &&
                legalMoves > (isPv ? 5 : 3) + !improving && !s.isCapture(m)) {
        ...
        if (legalMoves == 1) {
            ...
            if (score > alpha) {
                score = -search(c, si, gi, std::max(d, depth - 1), ply + 1, -alpha - 1,
                                -alpha, false, isNull);
            }
...
void parallel_search(Position s, SearchInfo si, int depth, int alpha, int beta,
                     int t) {
    ...
    for (int d = 1; !si.quit && d < MAX_PLY; ++d) {
        ...
        if (d > 4) {
            for (int i = 1; i < NUM_THREADS; ++i) {
                threads[i] = std::thread{parallel_search, s, si, d + (i & 1),
                                         NEG_INF, POS_INF, i};
            }

            results[0].first =
                search_root(s, si, global_info[0], d, 0, NEG_INF, POS_INF);
            THREAD_STOP = true;
        ...
        std::cout << " time " << si.clock.elapsed<std::chrono::milliseconds>()
                  << " nodes " << si.totalNodes << " nps "
                  << si.totalNodes /
                         (si.clock.elapsed<std::chrono::milliseconds>() + 1) * 1000;
        ...
        if ((si.depth && d >= si.depth) ||
            (si.max_nodes && si.totalNodes >= U64(si.max_nodes)) ||
            (si.nodes == si.prevNodes)) {
            break;
        }
