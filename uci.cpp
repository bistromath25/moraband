...
void go(std::istringstream &is, Position &s) {
    ...
    if (search_info.infinite) {
        ...
        search_info.moveTime = get_search_time(
            search_info.time[s.getOurColor()], search_info.inc[s.getOurColor()],
            global_info[0].history.size() / 2, search_info.moves_to_go,
            search_info.time[s.getOurColor()] -
                search_info.time[s.getTheirColor()]);
    }
...
void set_option(std::string &name, std::string &value) {
    if (name == "Hash") {
        ...
        D(std::cout << "Move Overhead set to value " << MOVE_OVERHEAD
                    << std::endl;);
    }
...
void bench(int depth) {
    ...
    double time =
        clock.elapsed<std::chrono::microseconds>() / static_cast<double>(1000000);
    std::cout << "Time:  " << time << std::endl;
    std::cout << "Nodes: " << totalNodes << std::endl;
    std::cout << "NPS:   " << U64(static_cast<long double>(totalNodes) / time)
              << std::endl;
}
...
void uci() {
    ...
    while (true) {
        ...
        if (token == "quit") {
            ...
            std::cout
                << "id name " << ENGINE_NAME << " " << ENGINE_VERSION << "\n"
                << "id author " << ENGINE_AUTHOR << "\n"
                << "option name Hash type spin default " << DEFAULT_HASH_SIZE
                << " min " << MIN_HASH_SIZE << " max " << MAX_HASH_SIZE << "\n"
