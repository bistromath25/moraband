...
const int SAFETY_TABLE[100] = {
    0, 0, 0, 1, 1, 2, 3, 4, 5, 6, 8, 10, 13, 16, 20,
    25, 30, 36, 42, 48, 55, 62, 70, 80, 90, 100, 110, 120, 130, 140,
    ...
    600, 610, 620, 630, 640, 650, 650, 650, 650, 650, 650, 650, 650, 650, 650,
...
struct PawnEntry {
    ...
    const std::array<Score, PLAYER_SIZE> &getMaterial() const { return material; }
    void clear() {
        key = 0;
...
struct PawnHashTable {
    ...
    ~PawnHashTable() { delete[] table; }
    void clear() {
        for (int i = 0; i < PAWN_HASH_SIZE; ++i) {
            table[i].clear();
        }
    }
    PawnEntry *probe(U64 key) { return &table[key % size]; }
    void store(U64 key, const std::array<Score, PLAYER_SIZE> &structure,
               const std::array<Score, PLAYER_SIZE> &material) {
